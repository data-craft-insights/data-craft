...
def create_airflow_session(airflow_url: str, username: str, password: str) -> Optional[requests.Session]:
    ...
    Tests authentication by hitting multiple endpoints
    """
    ...
    # Test 1: Health endpoint (may not require auth)
    ...
    # Test 2: DAGs list endpoint (requires auth)
    print("  Testing DAGs list endpoint...")
    dags_url = f"{base_url}/api/v1/dags"
    try:
        response = session.get(dags_url, auth=(username, password), timeout=10)
        if response.status_code == 200:
            print("✓ Authentication successful (DAGs endpoint accessible)")
            return session
        elif response.status_code == 401:
            print(f"✗ Authentication failed (401) - Invalid credentials")
            return None
        elif response.status_code == 403:
            print(f"✗ Authentication succeeded but access forbidden (403)")
            print("    User may lack permissions to access DAGs")
            return None
        else:
...
def trigger_dag(session: requests.Session, airflow_url: str, dag_id: str, username: str, password: str) -> Optional[str]:
    ...
    try:
        ...
        print(f"  Checking if DAG exists...")
        dag_check = session.get(dag_url, auth=(username, password), timeout=10)
        
        if dag_check.status_code == 404:
            print(f"✗ DAG '{dag_id}' not found")
            print(f"  Please verify the DAG ID is correct")
            return None
        elif dag_check.status_code == 200:
            dag_info = dag_check.json()
            is_paused = dag_info.get('is_paused', True)
            print(f"  DAG found - Paused: {is_paused}")
            if is_paused:
                print(f"  ⚠ Warning: DAG is paused, it may not run")
        elif dag_check.status_code == 401:
            print(f"✗ Authentication failed when checking DAG")
            return None
        elif dag_check.status_code == 403:
            print(f"✗ Permission denied - user lacks access to DAG")
            return None
        
        # Now trigger the DAG
        print(f"  Triggering DAG run...")
        ...
        if response.status_code == 401:
            ...
            print(f"  User may lack permission to trigger DAG runs")
            print(f"  Response: {response.text}")
            return None
        elif response.status_code == 404:
            print(f"✗ DAG not found (404)")
            return None
...
def main():
    ...
    if not session:
        ...
        print("  4. Check that the user has necessary permissions (Admin or Op role)")
        return 1
    ...
    if not dag_run_id:
        ...
        print("\nTroubleshooting:")
        print("  1. Check that the DAG exists and is enabled in Airflow UI")
        print("  2. Verify the DAG ID is correct: model_pipeline_with_evaluation")
        print("  3. Ensure the DAG is not paused (toggle in Airflow UI)")
        print("  4. Check Airflow logs for any DAG trigger errors")
        print("  5. Verify user has 'can_create' permission for DAG runs")
        return 1
